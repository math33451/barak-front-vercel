    ...
    { key: 'name', title: 'Nome da Unidade' },
    {
      ...
      render: (unit) => (
        <div className="flex gap-2">
          <button onClick={() => openModal(unit)} className="btn btn-sm btn-outline">
            <Edit className="h-4 w-4" />
          </button>
          <button onClick={() => handleDeleteUnit(unit.id)} className="btn btn-sm btn-outline btn-error">
  ...
  return (
    <DashboardLayout title="Unidades" activePath="/unidades">
      <div className="max-w-4xl mx-auto">
        ...
        {units.length > 0 ? (
          <DataTable<UnidadeEmpresaDTO> columns={columns} data={units} title="Lista de Unidades" />
        ) : (
